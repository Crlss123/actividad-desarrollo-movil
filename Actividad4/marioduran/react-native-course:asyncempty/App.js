...
const accountSettings = () => {
  ...
  const loadPreferences = async () => {
    try {
      ...
      const marketingEmailValue = await AsyncStorage.getItem(
        MARKETING_EMAILS_KEY
      );
      const latestNewsValue = await AsyncStorage.getItem(LATEST_NEWS_KEY);
      const darkThemeValue = await AsyncStorage.getItem(DARK_THEME_VALUE);

      if (pushNotiValue !== null)
        setPushNotifications(JSON.parse(pushNotiValue));
      if (marketingEmailValue !== null)
        setMarketingEmails(JSON.parse(marketingEmailValue));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <Switch
          ...
          onValueChange={async (value) => {
            setDarkTheme(value);
            await AsyncStorage.setItem(DARK_THEME_VALUE, JSON.stringify(value));
          }}
        />
      </View>
    </View>
